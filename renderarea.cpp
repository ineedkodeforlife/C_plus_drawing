    ...
    : QWidget{parent}
{
    ...
    pShapes.emplace_back(new Circle(QPoint(100, 40), 30, brush, pen));
    pShapes.emplace_back(new Square(QPoint(180, 200), 30, brush, pen));
    pShapes.emplace_back(new Triangle(
        { QPoint(100, 130), QPoint(150, 130), QPoint(125, 170) },
        brush, pen
    ));
    ...
    pMenu->addAction("Информация",this,SLOT(print_info()));
}
...
void RenderArea::highlightShapeByPoint(const QPoint &p)
{
    ...
    for (const auto &item : pShapes) {
        if (item->isPointInside(p) && !isShapeHighlighted) {
            ...
            item->setBrushColor(Qt::black);
    }
}

//void RenderArea::print_info(){
